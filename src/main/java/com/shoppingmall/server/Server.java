...
public class Server {
	...
	public void startServer(String IP, int port) {
		...
		try {
			...
			this.serverSocket.bind(new InetSocketAddress(IP, port));// 서버 컴퓨터 역할을 수행하는 그 컴퓨터가 자신의 IP주소 그리고 포트번호로 특정한
																	// 클라이언트의 접속을 기다림
				...
				while (true) {
					try {
						...
						System.out.println("[클라이언트 접속]" + socket.getRemoteSocketAddress() + ": "
								+ Thread.currentThread().getName());// 현재 접속을 한 클라이언트의 IP주소와 같은 주소 정보를 출력하고 스레드의
																	// 고유정보(이름) 출력
					} catch (Exception e) {
		...
		executorService.submit(runnable); // 스레드 풀에 현재 클라이언트를 기다리는 runnable객체를 담을 수 있도록 처리를 해서 스레드 풀을 먼저 초기화를 해주고 스레드
											// 풀안에 첫번째 스레드로써 클라이언트에 접속을 기다리는 스레드를 넣어준것
	}
